...
use metamesh_transport::{TransportMonitor, MetaMeshPacket, PacketHeader, PacketType, PacketFlags, MAGIC_HEADER};
use rand::RngCore;
use std::process;
use std::sync::Arc;
...
mod storage;
use config::Config;
use storage::{SecureStorage, StoredIdentity, QueuedPacket, get_current_timestamp};
...
impl AppState {
    fn new(transport_monitor: Arc<Mutex<TransportMonitor>>) -> Self {
        ...
        Self { _config: config, storage, transport_monitor }
    }
}
...
async fn retry_pending_packets(storage: &SecureStorage, transport_monitor: &Arc<Mutex<TransportMonitor>>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    for packet in pending_packets {
        ...
        if current_time - packet.last_retry >= 300 {
            println!("🔄 Retrying packet {} (attempt {})", packet.packet_id, packet.retry_count + 1);
...
impl MetaMeshService for MetaMeshServiceImpl {
    ...
    async fn create_address(&self, _request: Request<CreateAddressRequest>) -> Result<Response<CreateAddressResponse>, Status> {
        ...
        self.state.storage.store_identity(stored_identity)
    ...
    async fn recover_keys(&self, request: Request<RecoverKeysRequest>) -> Result<Response<RecoverKeysResponse>, Status> {
    ...
    async fn list_addresses(&self, _request: Request<ListAddressesRequest>) -> Result<Response<ListAddressesResponse>, Status> {
        match self.state.storage.list_identities() {
            Ok(identities) => {
                let addresses: Vec<AddressInfo> = identities.into_iter().map(|id| AddressInfo {
                    seed_id: id.seed_id,
    ...
    async fn delete_address(&self, request: Request<DeleteAddressRequest>) -> Result<Response<DeleteAddressResponse>, Status> {
        let req = request.into_inner();
    ...
    async fn delete_all_addresses(&self, _request: Request<DeleteAllAddressesRequest>) -> Result<Response<DeleteAllAddressesResponse>, Status> {
    ...
    async fn ping_check(&self, request: Request<PingCheckRequest>) -> Result<Response<PingCheckResponse>, Status> {
        let req = request.into_inner();
        ...
        self.state.storage.queue_packet(queued_packet)
        ...
        let send_results = transport_monitor.send_to_all_transports(&packet_bytes).await;
        
        let mut message = format!("Ping packet created for seed_id: {} ({} bytes)\n", req.seed_id, packet_bytes.len());
    ...
    async fn deserialize(&self, request: Request<DeserializeRequest>) -> Result<Response<DeserializeResponse>, Status> {
        let req = request.into_inner();
        ...
        let from_seed_str = String::from_utf8_lossy(&packet.header.from_seed).trim_end_matches('\0').to_string();
        let to_seed_str = String::from_utf8_lossy(&packet.header.to_seed).trim_end_matches('\0').to_string();
        let to_seed_display = if to_seed_str.is_empty() { "BROADCAST".to_string() } else { to_seed_str };
    ...
    async fn pending_packets(&self, _request: Request<PendingPacketsRequest>) -> Result<Response<PendingPacketsResponse>, Status> {
        match self.state.storage.get_pending_packets() {
            Ok(packets) => {
                let packet_infos: Vec<PacketInfo> = packets.into_iter().map(|p| PacketInfo {
